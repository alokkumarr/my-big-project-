...
public class Id3SecurityController {
    ...
    @Autowired
    public Id3SecurityController(ExternalSecurityService securityService,
    ...
    public RoleCatPrivilegeResponse createRoleCategoryPrivilege(
        ...
        @RequestHeader("Authorization") String id3token) throws IOException {
        RoleCatPrivilegeResponse response = new RoleCatPrivilegeResponse();
        if (request == null) {
            logger.warn("Request body can't be blank or empty.");
            httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),"Request body can't be blank or empty.");
            return null;
        }

        if ("ALERTS".equalsIgnoreCase(request.getModuleName())
            || "WORKBENCH".equalsIgnoreCase(request.getModuleName())) {
            httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),"ALERTS and WORKBENCH module are not allowed.");
            return null;
        }
        ...
        RoleType roleType = RoleType.fromValue(id3User.getRoleType());
        ...
        if (role.getCustomerCode() == null || role.getCustomerCode().isEmpty()) {
            httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),
                "Customer Code can't be blank or empty.");
            return null;
        }

        if (role.getRoleType() == null || role.getRoleType().isEmpty()) {
            httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),
                "Role Type can't be blank or empty.");
            return null;
        }
        ...
        if (!RoleType.validRoleType(role.getRoleType())) {
            ...
            httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),
                "Only ADMIN|USER Role Type are allowed.");
            return null;
        }
        ...
        if (role.getRoleName() == null || role.getRoleName().trim().isEmpty()) {
            ...
            httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),
                "Role Name can't be blank or empty.");
            return null;
        }
        ...
        if (customerSysId == null || customerSysId == 0) {
            ...
            httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),
                "Product and Module does not exist for this user.");
            return null;
        }
        ...
        if (validateRoleName || role.getRoleName().trim().contains(" ")) {
            logger.error("Special symbol and numeric not allowed except" +
            ...
            if (emptyCategoryName) {
                ...
                httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),
                    "Category name can't be blank or empty.");
                return null;
            }
            ...
            if (invalidCategoryName) {
                logger.error("Special symbol not allowed except " +
                ...
                for (CategoryDetails categoryDetails : categoryList) {
                    ...
                    if (subCategoryList != null && !subCategoryList.isEmpty()) {
                        ...
                        if (emptySubCategoryName) {
                            ...
                            httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),
                                "Sub Category name can't be blank or empty");
                            return null;
                        }
                        invalidSubCatName[0] =
                            subCategoryList.stream()
        ...
        if (subCategoryList != null && !subCategoryList.isEmpty() && subCategoryList.size() > 0) {
            for (SubCategoryDetails details : subCategoryList) {
                if (request.getModuleName() != null
                    ...
                    logger.error("Please provide the valid module " +
                        "privileges for category/subcategory.");
                    httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),
                        "Please provide the valid module " +
                            "privileges for category/subcategory.");
                    return null;
                }
            }
        ...
        if (!role.getCustomerCode().equalsIgnoreCase(moduleDetails.getCustomerCode())) {
            ...
            httpResponse.sendError(HttpStatus.BAD_REQUEST.value(),"Customer Code not matched with the user ticket.");
            return null;
        }
    ...
    public RoleCatPrivilegeResponse fetchRoleCategoryPrivilege(
        ...
        Id3User user = id3Repository.getId3Userdetails(id3Claims.getMasterLoginId());
        RoleType roleType = RoleType.valueOf(user.getRoleType());
        String masterLoginId = user.getUserId();
        if (masterLoginId != null
            || !RoleType.ADMIN.equals(roleType)) {
        ...
        final Long customerSysId = moduleDetails != null ? moduleDetails.getCustomerSysId() : null;
        if (customerSysId == null || customerSysId == 0) {
            httpResponse.sendError(HttpStatus.INTERNAL_SERVER_ERROR.value(),"Role Name can't be blank or empty.");
            response.setValid(false);
    ...
    public UsersDetailsList getUserList(HttpServletRequest request, HttpServletResponse response,
       ...
       // Ticket ticket = SipCommonUtils.getTicket(request);
        ...
        if (!usersDetailsList.getValid()) {
            response.sendError(HttpStatus.INTERNAL_SERVER_ERROR.value(), usersDetailsList.getValidityMessage());
            return null;
        }
    ...
    public Object getAllSecGroupDetailsForCustomer(
        ...
        if (id3Claims==null)
        {
            ...
            valid.setValidityMessage("Invalid Token");
            return valid;
        }
        Long customerId = id3User.getCustomerSysId();
        RoleType roleType = RoleType.ADMIN;
