...
public class LoggingAspects {
  ...
  /**
   * This is point cut method.
   */
  @Around("com.synchronoss.saw.gateway.aspects."
      + "RoutingArchitecture.mediaAnnotation() || "
      + "com.synchronoss.saw.gateway.aspects."
      + "RoutingArchitecture.inRouteLayer() "
      + "|| com.synchronoss.saw.gateway.aspects.RoutingArchitecture.controller()")
  public Object inRouteAroundAdvice(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
    ...
    if (method.getDeclaringClass().isInterface()) {
      ...
      try {
        method = proceedingJoinPoint.getTarget().getClass().getDeclaredMethod(
            proceedingJoinPoint.getSignature().getName(), method.getParameterTypes());
      } catch (final SecurityException exception) {
        LOGGER.error("Exception occurred while tracing the method at"
            + this.getClass().getMethods()[0] + " :", exception);
    ...
    try {
      ...
      stringBuffer.append("() ").append(" of the class :")
      ...
      if (returnedValueFromMethod != null) {
        stringBuffer.append(" Returning Value :").append(returnedValueFromMethod);
        stringBuffer.append(System.getProperty("line.separator"));
      }
      stringBuffer.append(" Method : ").append(method.getName());
      stringBuffer.append("() ").append(" of the class :")
          .append(proceedingJoinPoint.getTarget().getClass());
      stringBuffer.append(" with the arguments :")
          .append(Arrays.toString(proceedingJoinPoint.getArgs())).append(" ends here ");
      LOGGER.debug(stringBuffer.toString());
      stringBuffer = null;
    } catch (HttpServerErrorException nestedException) {
      LOGGER.error(
          "An Exception has been thrown in :" + proceedingJoinPoint.getSignature().getName());
      LOGGER.error("Cause: " + nestedException.fillInStackTrace());
