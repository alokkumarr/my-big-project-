...
public class ExtractHelper {
  ...
  public static int isDateLieral(String token) {
    ...
    if (isWeekDayLiteral(s)) {
        return TrieTree.WEEKDAY_LITERAL;
    }
    if (isMonthLiteral(s)) {
        return TrieTree.MONTH_LITERAL;
    }
    return TrieTree.NOT_DATE_LITERAL;
}
...
private static boolean isMonthLiteral(String s) {
    ...
    for (String c : TrieTree.MONTH_SHORT) {
        if (s.equals(c)) {
            return true;
        }
    }
    for (String c : TrieTree.MONTH_FULL) {
        if (s.equals(c)) {
            return true;
        }
    }
 ...
 * @return
 */
public static boolean hasFullMonthLiteral(String s) {
    s = s.toLowerCase();
    for (String c : TrieTree.MONTH_FULL) {
        if (s.contains(c)) {
            return true;
...
private static boolean isWeekDayLiteral(String s) {
    ...
    for (String c : TrieTree.WEEKDAY_SHORT) {
        if (s.equals(c)) {
            return true;
        }
    }
    for (String c : TrieTree.WEEKDAY_FULL) {
        if (s.equals(c)) {
            return true;
        }
    }
...
public static List<String> testDataForDateFormats() {
    ...
    for (String str : TrieTree.PATTERN) {
        String testData = "";
    ...
    for (String str : TrieTree.TIME_PATTERN) {
        String testData = "";
        for (int i = 0; i < str.length(); i++) {
            testData = testData + getTestDataForChar(str.charAt(i));
        }
