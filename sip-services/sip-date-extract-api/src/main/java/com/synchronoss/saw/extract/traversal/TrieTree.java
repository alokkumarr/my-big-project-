...
public class TrieTree {
  ...
  public static TemporalPredictTraversalNode monthPredictionTree = new TemporalPredictTraversalNode();
  public static TemporalPredictTraversalNode weekPredictionTree = new TemporalPredictTraversalNode();
  public static TemporalPredictTraversalNode patternPredictionTree = new TemporalPredictTraversalNode();
  public static TemporalPredictTraversalNode timePredictionTree = new TemporalPredictTraversalNode();
  ...
  public static void buildTree(List<String> itemList, TemporalPredictTraversalNode parentNode) {
    for (String s : itemList) {
      TemporalPredictTraversalNode parent = parentNode;
      parentNode.charcter = 0;
      for (int i = 0; i < s.length(); i++) {
        char c = s.charAt(i);
        TemporalPredictTraversalNode node = parent.getChild(c);
        if (node == null) {
          TemporalPredictTraversalNode child = new TemporalPredictTraversalNode();
          child.charcter = c;
          if (i + 1 == s.length()) {
  ...
  public static String getProbables(TemporalPredictTraversalNode tree) {
    if (tree == null) {
      return "";
    }
    List<TemporalPredictTraversalNode> kids = tree.childern;
    String pattern = tree.charcter + " -->";
    for (TemporalPredictTraversalNode kid : kids) {
      pattern = pattern + " " + kid.charcter;
    }
    return pattern;
  }

  public static void printTree(TemporalPredictTraversalNode tree) {

    tree = sortTree(tree);

    ExhibitTrieTree displyObject = displayBuilder(new ExhibitTrieTree(), tree);
    ...
    int prevLevel = 0;
    for (TrieTreeSymbols tc : displyObject.displayBuffer) {
      if (prevLevel == tc.level) {
        line = line.substring(0, tc.leftSpace) + tc.symbol
            + line.substring(tc.leftSpace + tc.symbol.length());
      } else {
   ...
   * @return
   */
  private static ExhibitTrieTree displayBuilder(ExhibitTrieTree display, TemporalPredictTraversalNode tree) {
    if (isFamilyUnit(tree)) {
      return familyDisplayBuilder(tree);
    }

    ExhibitTrieTree horizontalDisply = new ExhibitTrieTree();
    for (TemporalPredictTraversalNode kid : tree.childern) {
      ExhibitTrieTree recusiveDisplay = displayBuilder(horizontalDisply, kid);
      horizontalDisply = adjustHorizontalDisplay(horizontalDisply, recusiveDisplay);
    }

    Collections.sort(horizontalDisply.displayBuffer);

    ExhibitTrieTree verticalDisplay = new ExhibitTrieTree();
    int[] dadJoints = new int[tree.childrenCount()];

    int j = 0;
    for (TrieTreeSymbols tc : horizontalDisply.displayBuffer) {
    ...
    if (tree.childrenCount() == 1) {
      String s = tree.explictDateFragment ? String.valueOf(tree.charcter) + TERMINATION_SYMBOL
          : String.valueOf(tree.charcter);
      TrieTreeSymbols tc = new TrieTreeSymbols(s, dadJoints[0], 0);
      ...
      String symbol = "_";
      int papaPosition =
          dadJoints.length % 2 == 0 ? 1 + (dadJoints[dadJoints.length - 1] + dadJoints[0]) / 2
              : dadJoints[dadJoints.length / 2] + 1;
      ...
      for (int k = dadJoints[0] + 1; k <= dadJoints[dadJoints.length - 1] + 1 - 2; k++) {
        ...
        if (k == papaPosition) {
          String s =
              "".equals(String.valueOf(tree.charcter).trim()) ? "$" : String.valueOf(tree.charcter);
          if (tree.explictDateFragment) {
            s = s + TERMINATION_SYMBOL;
          }
          tc = new TrieTreeSymbols(s, k, 0);
        } else {
          tc = new TrieTreeSymbols(symbol, k, 0);
        }
        verticalDisplay.displayBuffer.add(tc);
      }
    }
    ...
    for (int i = 0; i < dadJoints.length; i++) {
      for (int m = 0; m < VERTICAL_PRINT_GAP; m++) {
        TrieTreeSymbols tc = new TrieTreeSymbols("|", dadJoints[i], m + 1);
        verticalDisplay.displayBuffer.add(tc);
      }
    }
  ...
  private static ExhibitTrieTree adjustVerticalDisplay(ExhibitTrieTree horizontalDisply,
      ExhibitTrieTree verticalDisplay) {
   ...
   * @return
   */
  private static ExhibitTrieTree adjustHorizontalDisplay(ExhibitTrieTree display,
      ExhibitTrieTree kidDisplay) {
    if (display.displayBuffer.size() == 0) {
      return kidDisplay;
    }

    int leftPos = display.width;
    for (TrieTreeSymbols tc : kidDisplay.displayBuffer) {
      tc.leftSpace += (leftPos + HOROZONTAL_PRINT_GAP);
      int i = 0;
      int displayBufferSize = display.displayBuffer.size();
      for (TrieTreeSymbols displayTc : display.displayBuffer) {
  ...
  private static ExhibitTrieTree familyDisplayBuilder(TemporalPredictTraversalNode tree) {
    ExhibitTrieTree displyObject = new ExhibitTrieTree();
    ...
    if (kids == 1) {
      displyObject.displayBuffer.add(new TrieTreeSymbols(charVal, 0, 0));
      displyObject.displayBuffer.add(new TrieTreeSymbols("|", 0, 1));
      String kidChar = String.valueOf(tree.childern.get(0).charcter);
      displyObject.displayBuffer.add(new TrieTreeSymbols(kidChar, 0, 2));
   ...
   * @return
   */
  public static ExhibitTrieTree evenKidCountTree(ExhibitTrieTree displyObject,
      TemporalPredictTraversalNode tree) {
    int kids = tree.childrenCount();
    ...
    displyObject.displayBuffer.add(new TrieTreeSymbols(charVal, papaPosition, 0));
    ...
    for (int i = 0; i < kids / 2; i++) {
      ...
      for (int j = 0; j < markers; j++) {
        ...
        if (j < markers / 2) {
          ...
          displyObject.displayBuffer.add(new TrieTreeSymbols(symbol, leftpos, i + 1));
        } else {
          ...
          displyObject.displayBuffer.add(new TrieTreeSymbols(symbol, rightpos, i + 1));
        }
      }
    ...
    for (int i = 0; i < kids; i++) {
      ...
      if (i < kids / 2) {
        displyObject.displayBuffer.add(new TrieTreeSymbols(kidVal, i, kids / 2 + 1));
      } else {
        displyObject.displayBuffer.add(new TrieTreeSymbols(kidVal, i + 1, kids / 2 + 1));
      }
    }
    return displyObject;
  }
   ...
   * @return
   */
  public static ExhibitTrieTree oddKidCountTree(ExhibitTrieTree displyObject,
      TemporalPredictTraversalNode tree) {
    int kids = tree.childrenCount();
    ...
    for (int i = 0; i < (kids - 1) / 2; i++) {
      ...
      for (int j = 0; j < markers; j++) {
        ...
        if (j < markers / 2) {
          ...
          displyObject.displayBuffer.add(new TrieTreeSymbols("|", papaPosition, i + 1));
        } else {
          int rightpos = papaPosition + coeffiecient++;
          // String symbol = "\\";
          String symbol = "|";
  ...
  private static int getMaxTreeHeight(TemporalPredictTraversalNode tree) {
    if (tree.hasChildern()) {
      int[] height = new int[tree.childrenCount()];
      for (int i = 0; i < tree.childrenCount(); i++) {
        TemporalPredictTraversalNode child = tree.childern.get(i);
  ...
  public static TemporalPredictTraversalNode sortTree(TemporalPredictTraversalNode tree) {
    if (tree.hasChildern()) {
      for (TemporalPredictTraversalNode child : tree.childern) {
      ...
      while (swapped) {
        ...
        for (int i = 0; i < tree.childrenCount() - 1; i++) {
          ...
          if (thisHeight < nextHeight) {
            swapped = true;
            TemporalPredictTraversalNode tempChild = tree.childern.get(i);
   ...
   * @return
   */
  private static boolean isFamilyUnit(TemporalPredictTraversalNode tree) {
    if (tree.hasChildern()) {
      for (TemporalPredictTraversalNode node : tree.childern) {
        if (node.hasChildern()) {
          return false;
        }
      }
  ...
  public static String getRegexPattern(List<String> treeElements) {
    TemporalPredictTraversalNode tree = new TemporalPredictTraversalNode();
  ...
  private static String getRegexPattern(TemporalPredictTraversalNode tree) {
    if (isFamilyUnit(tree)) {
  ...
  private static String getFamilyUnitPattern(TemporalPredictTraversalNode tree) {
    StringBuilder buffer = new StringBuilder();
