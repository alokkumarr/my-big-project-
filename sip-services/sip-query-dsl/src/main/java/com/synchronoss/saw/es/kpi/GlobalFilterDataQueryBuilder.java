...
public class GlobalFilterDataQueryBuilder {
  ...
  private static final Logger logger = LoggerFactory.getLogger(GlobalFilterDataQueryBuilder.class);
   ...
   */
  public List<GlobalFilterExecutionObject> buildQuery(
      GlobalFilters globalFilters, SipDskAttribute dskAttribute) {
  ...
  private List<AggregationBuilder> filterAggregationBuilder(Filter filter) {
    ...
    BucketOrder sortOrder =
        (filter.getOrder() != null && filter.getOrder().equals(Order.ASC.name()))
            ? BucketOrder.key(true)
            : BucketOrder.key(false);
    switch (filter.getType()) {
      case INT:
        aggregationBuilder =
            AggregationBuilders.min(filter.getColumnName() + "_min").field(filter.getColumnName());
        aggregationBuilder1 =
            AggregationBuilders.max(filter.getColumnName() + "_max").field(filter.getColumnName());
      ...
      case STRING:
        aggregationBuilder =
            AggregationBuilders.terms(filter.getColumnName())
