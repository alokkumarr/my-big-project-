...
public class NGContextServices implements WithDataSet, WithProjectScope{
    ...
    public static ComponentConfiguration analyzeAndValidateTransformerConf(String configAsStr) {
            ...
            if (!valid) throw new XDFException(ReturnCode.CONFIG_ERROR, "Incorrect configuration: dataset parameter with name 'input' does not exist .");
            ...
            if (!valid ) throw new XDFException(ReturnCode.CONFIG_ERROR, "Incorrect configuration: dataset parameter with name 'output' does not exist .");
            return compConf;
    ...
    public static ComponentConfiguration analyzeAndValidateSqlConf(String cfgAsStr){
        ...
        if (SQLProps == null) {
            throw new XDFException(ReturnCode.NO_COMPONENT_DESCRIPTOR, "sql");
        }
        if (SQLProps.getScript() == null || SQLProps.getScript().isEmpty()) {
            throw new XDFException(ReturnCode.CONFIG_ERROR, "Incorrect configuration: Spark SQL does not have SQL script name.");
        }
        if (SQLProps.getScriptLocation() == null || SQLProps.getScriptLocation().isEmpty()) {
            throw new XDFException(ReturnCode.CONFIG_ERROR, "Incorrect configuration: Spark SQL descriptor does not have SQL script location.");
        }
        return compConf;
    }
    ...
    public static ComponentConfiguration analyzeAndValidateEsLoaderConf(String cfgAsStr){
        ...
        if (esLoaderConfig == null)
            throw new XDFException(ReturnCode.NO_COMPONENT_DESCRIPTOR, "es-loader");

        if (esLoaderConfig.getEsNodes() == null || esLoaderConfig.getEsNodes().isEmpty()) {
            throw new XDFException(ReturnCode.CONFIG_ERROR, "Incorrect configuration: ElasticSearch Nodes configuration missing.");
        }
        if (esLoaderConfig.getEsPort() == 0) {
            throw new XDFException(ReturnCode.CONFIG_ERROR, "Incorrect configuration: ElasticSearch Port configuration missing.");
        }
        if (esLoaderConfig.getDestinationIndexName() == null || esLoaderConfig.getDestinationIndexName().isEmpty()) {
            throw new XDFException(ReturnCode.CONFIG_ERROR, "Incorrect configuration: ElasticSearch Destination Index Name missing.");
        }
        if (esLoaderConfig.getEsClusterName() == null || esLoaderConfig.getEsClusterName().isEmpty()) {
            throw new XDFException(ReturnCode.CONFIG_ERROR, "Incorrect configuration: ElasticSearch clustername configuration missing.");
        }
        return compConf;
    }
