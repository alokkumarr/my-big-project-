...
larg_DS <-  read.csv(file="C:/Git_Workspace/a2modeler/tests/testthat/Salaries-small_set.csv", header=TRUE, sep=",")

int_data <- larg_DS %>% select (.,Id,BasePay,OvertimePay,OtherPay,TotalPay,TotalPayBenefits,MaritalStatus)
...
f1_class <- new_classifier(df = spk_df,target = "MaritalStatus", name = "Classifier-constructer",desc = "New classifier")%>%
  add_model(pipe = NULL,
...
test_pipe <- pipeline(expr = function(x){select(x, index, MaritalStatus, BasePay)})
...
test_that("Classifier constructor New Data", {
  ...
  f2_preds <- predict(f1_class, data = spk_df, periods = 10, level = c(80, 95))
