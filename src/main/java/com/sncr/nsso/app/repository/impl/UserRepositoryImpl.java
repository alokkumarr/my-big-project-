	...
	public void prepareTicketDetails(User user, Boolean onlyDef) {
		...
		try {
			...
			String sql5 = "	SELECT U.USER_SYS_ID, U.CUSTOMER_SYS_ID,C.CUSTOMER_SYS_ID,CP.CUST_PROD_SYS_ID,CP.CUSTOMER_SYS_ID,CPMF.CUST_PROD_MOD_FEATURE_SYS_ID,"
					+ " P.PRODUCT_CODE ,M.MODULE_CODE,CPMF.FEATURE_NAME,CPMF.FEATURE_DESC,CPMF.DEFAULT_URL,CPMF.DEFAULT FROM USERS U INNER JOIN"
					+ " CUSTOMERS  C ON (C.CUSTOMER_SYS_ID=U.CUSTOMER_SYS_ID) INNER JOIN CUSTOMER_PRODUCTS CP ON (CP.CUSTOMER_SYS_ID=C.CUSTOMER_SYS_ID)"
					...
					+ " INNER JOIN PRODUCT_MODULES PM ON (PM.PROD_MOD_SYS_ID=CPM.PROD_MOD_SYS_ID) INNER JOIN MODULES M ON(M.MODULE_SYS_ID=PM.MODULE_SYS_ID)"
			...
			String sql2 = "SELECT P.PRIVILEGE_CODE, P.PRIVILEGE_NAME, P.PRIVILEGE_DESC, CPMF.FEATURE_NAME FROM CUSTOMER_PRODUCT_MODULE_FEATURES CPMF"
					+ " INNER JOIN `PRIVILEGES` P ON (P.CUST_PROD_MOD_FEATURE_SYS_ID = CPMF.CUST_PROD_MOD_FEATURE_SYS_ID) INNER JOIN ROLE_PRIVILEGES RP"
					+ " ON(RP.PRIVILEGE_SYS_ID = P.PRIVILEGE_SYS_ID AND RP.ACTIVE_STATUS_IND = P.ACTIVE_STATUS_IND) INNER JOIN CUSTOMER_PRODUCT_MODULES CPM"
					+ " ON(CPM.CUST_PROD_MOD_SYS_ID = CPMF.CUST_PROD_MOD_SYS_ID) INNER JOIN CUSTOMER_PRODUCTS CP ON(CP.CUST_PROD_SYS_ID = CPM.CUST_PROD_SYS_ID)"
					+ " INNER JOIN USERS U ON(U.ROLE_SYS_ID=RP.ROLE_SYS_ID AND U.CUSTOMER_SYS_ID = CP.CUSTOMER_SYS_ID)"
					+ " WHERE CP.ACTIVE_STATUS_IND = CPM.ACTIVE_STATUS_IND AND CPM.ACTIVE_STATUS_IND = CPMF.ACTIVE_STATUS_IND"					
