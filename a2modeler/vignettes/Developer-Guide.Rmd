...
A key design principle of a2modeler is the set of high level functions common to multiple use cases seen in the image below:
...
The a2modeler workflow starts with a use-case specific function with creates a modeler class object (discussed below). Each of use-case constructor function creates a separate child class object that inherits from the parent modeler class. The actual `modeler` constructor is only used internally by the use case specific constructors. The use of class inheritance allows for general code to be shared across sub-classes but specific constraints, parameters, and methods to be implemented for each use case. 
...
Next is the `classifier` constructor. The function is very similar to the `regressor` except now there are checks for a binary distribution of the target variable and AUC default measure.
...
getS3method("add_default_samples", "modeler")
```

The other `add_*_samples` modeler methods behave similarly. The numeric methods is where the sampling logic is located. 
...
* __train_indices__: list of numeric train indices. each index contains numeric vector pertaining to row numbers to use for model fitting
* __validation_indices__: list of numeric validation indices. each index contains numeric vector pertaining to row numbers to use for model validation
* __indices_names__: character vector of indices names
* __test_index__:  numeric index with records to use for model testing 

In the following chunk code for `add_default_samples` methods are shown. The data.frame and tbl_spark methods use the numeric method internally to reduce code duplication. The default sample is the simplest strategy and only a single row based indie is saved in the train_indices list. 

```{r}
# add_default_samples.numeric code
getS3method("add_default_samples", "numeric")

# add_default_samples.data.frame code
getS3method("add_default_samples", "data.frame")

# add_default_samples.tbl_spk
getS3method("add_default_samples", "tbl_spark")

```


Next is a set of methods for creating holdout samples. The numeric method creates train and validation row indices based on the splits argument. 


```{r}
# add_holdout_samples.numeric code
getS3method("add_holdout_samples", "numeric")

# add_holdout_samples.data.frame code
getS3method("add_holdout_samples", "data.frame")

# add_holdout_samples.data.frame code
getS3method("add_holdout_samples", "tbl_spark")

```


The concept here is that the sampling strategy is set once, creating indices that control the data partitions for all down stream modeling fitting and evaluating. The samples object is saved in the modeler object and referenced internally for data partitioning. More on its implementation later on. 
...
* __get_train_samples__: returns training indices
* __get_validation_samples__: returns the validation indices
* __get_indices__: returns a list with an element for each train and validation indices pairs
...
The pipeline class encapsulates data transformation logic to support repeatable feature generation in a2model development. The pipeline object stores the transformation logic, some meta-data, and potentially pipeline output. Appending transformation output to the pipeline is useful for cases when that output is used more than once.
...
One or more pipelines can be created and stored in a modeler object. They are stored as a named list of pipelines in the pipelines slot. The name is the pipeline uid. The re-usability concept is useful internally to the modeler object so that models can share pipeline output (and not require duplicate execution and storage).
...
getS3method("train_models", "modeler")
...
The `execute_pipelines` function is descriptive - it calls the `execute` function on all un-executed pipelines saved in the modeler object. Recall execute will save the pipeline output to the output slot. 


```{r}
# execute pipeline modeler method
getS3method("execute_pipelines", "modeler")
```
...
The next step is the train a single model uid with the `train_model` function. The `train_model` has methods for specific modeling backends and discussed in-depth in the following sections. The model output from the train_model function is stored on the modeler model list and the model performance is added to the consolidated modeler performance. 
...
getS3method("train_model", "spark_model")
...
In the next section, a hack is made for using the spark-ml neural net functions. They require a valid layers input that is made separately from the param_map
...
getS3method("train_model", "forecast_model")
...
Once a modeler workflow is completed predictions can be made on new datasets similar to the input data. In the a2modeler package the prediction class supports prediction management. Below is the constructor function for creating a predictions object. The class stores information on the predictions, the model used to make the predictions, the modeler type (class), and some meta-data.
...
getS3method("predict", "classifier")
```


The predict method used internally is specific to the final model class. In the classifier case, the internal predict method below uses the sparklyr `ml_predict` function on the spark-ml model and then formatted to the expected output schema. 

```{r}
# Spark Model Classification predict method
getS3method("predict", "spark_model_classification")
```
...
getS3method("predict", "forecast_model")

# get_forecasts
getS3method("get_forecasts", "forecast")
```
...
Below is the refit modeler method. Similar to the prediction methods, the refit applies assertions on the new data ensuring its the same class and schema of the original modeler input data. The final model pipeline is applied with a single default sample to create the training dataset and passed to the `train_model` function. 


```{r}
# Refit Method
getS3method("refit", "modeler")
```


#### Deploy

The deploy function can be used to save a modeler object to file for use on a new dataset in a different time and environment. 

The function below does the following:

* Creates a label from the modeler name and version
* Creates a model path using the label and model uid
* Removes all spark objects from the modeler objects which should be saved separately
