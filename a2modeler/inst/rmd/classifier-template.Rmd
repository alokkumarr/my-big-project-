...
# R packages
packages <- c("a2munge", "a2modeler", "a2charter", "sparklyr", "dplyr", "purrr", "ggplot2", "rlang", "tidyr")
...
```{r leaderboard}
c1$performance %>% 
  dplyr::select(model_uid, pipeline_uid, method, param_grid, sample, !!c1$measure$method) %>% 
  dplyr::arrange_at(c1$measure$method) %>% 
  knitr::kable(.,
               digits = 3,
...
if(! is.null(c1$samples$test_holdout_prct)) {
  ...
  # count table
  t1 <- tpdf %>% 
    dplyr::select(!!sym(pred_var), !!sym(c1$target)) %>% 
    table(.)
  addmargins(t1, c(1,2))
  
  # prop table
  round(addmargins(prop.table(t1), c(1,2)), 3)
  ...
  cm <- tpdf %>% 
    ...
    a2munge::expander(., fun = funs(label = c("TN", "FN", "FP", "TP"))) %>% 
    tidyr::replace_na(list(n = 0)) %>% 
    tidyr::spread(key = "label", value = "n", drop=T, fill=0)
...
if(! is.null(c1$samples$test_holdout_prct)) {
  ...
  roc %>%
   a2charter::gg_line_chart("fpr", "tpr", 
                 title = "ROC Curve",
     ...
     ggplot2::geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1),
...
if(! is.null(c1$samples$test_holdout_prct)) {
  ...
  bin_n <- 10
  gains <- roc %>% 
    dplyr::mutate(bin = ntile(n, bin_n)) %>% 
    dplyr::group_by(bin) %>% 
    dplyr::summarise(n  = n(),
                     tp = sum(!!sym(c1$target))) %>% 
    dplyr::ungroup() %>% 
    dplyr::mutate(decile = cumsum(n) / sum(n),
                  cumtp  = cumsum(tp), 
                  gain   = cumtp / sum(tp),
                  lift   = gain / decile)
  
  p1 <- a2charter::gg_line_chart(gains, 
                                 x_variable = "decile",
                                 y_variable = "gain",
                                 points = TRUE,
    ...
    ggplot2::geom_ribbon(aes(ymin = decile, ymax = gain), fill = sncr_cols(1), alpha=.2) +
    ggplot2::geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1),
                         linetype = 5, color = "grey50", size = 1.05) +
    ggplot2::scale_x_continuous(limits = c(0, 1), breaks = round(gains$decile, 2)) +
    ggplot2::scale_y_continuous(limits = c(0, 1))
  
  p2 <- a2charter::gg_line_chart(gains, 
                                 x_variable = "decile",
                                 y_variable = "lift",
                                 color = sncr_cols(9),
                                 points = TRUE,
                                 ...
                                 caption = "Sample ordered by positive class probability") +
    ggplot2::geom_ribbon(aes(ymin = 1, ymax = lift), fill = sncr_cols(9), alpha=.2) +
    ggplot2::scale_x_continuous(limits = c(0, 1), breaks = round(gains$decile, 2)) 
  
  gridExtra::grid.arrange(p1, p2, ncol = 2)
  
  gains %>% 
    dplyr::rename(!!quo_name(paste('Cumulated', c1$target)) := cumtp,
           !!quo_name(c1$target) := tp) %>%
    dplyr::select(bin, decile, n,
...
```{r regressor-config, echo=TRUE, eval=FALSE, ref.label=c('inputs', 'classifier-creation', 'measure', 'sampling', 'feature-generation', 'models', 'train-models', 'set-final')}
