...
public class SawBisRouteController {
  ...
  @Autowired private BisRouteService bisRouteService;
  ...
  public ResponseEntity<@Valid BisRouteDto> createRoute(
    ...
    return ResponseEntity.ok(
        bisChannelDataRestRepository
            ...
            .map(
                channel -> {
                  ...
                  try {
                    ...
                    boolean isExists =
                        bisRouteService.isRouteNameExists(channelId, routeName.asText());
                    ...
                    throw new SftpProcessorException(
                        "Exception occurred while creating routeMetaData ", e);
                  }
                  ...
                  if (schedulerExpn != null) {
                    ...
                    schedulerRequest.setJobName(
                        channel.getChannelType()
                            + routeEntity.getBisChannelSysId()
                    ...
                    // }
                    logger.info(
                        "posting scheduler inserting uri starts here: "
                            + bisSchedulerUrl
                            + scheduleUri
                            + insertUrl);
                    restTemplate.postForLocation(
                        bisSchedulerUrl + scheduleUri + insertUrl, schedulerRequest);
                    logger.info(
                        "posting scheduler inserting uri ends here: "
                            + bisSchedulerUrl
                            + scheduleUri
                            + insertUrl);
                  }
            ...
            .orElseThrow(
                () -> new ResourceNotFoundException("channelId " + channelId + " not found")));
  }
   ...
   */
  @RequestMapping(
      value = "/channels/{channelId}/routes/{routeId}/deactivate",
      method = RequestMethod.PUT,
      produces = org.springframework.http.MediaType.APPLICATION_JSON_UTF8_VALUE)
  public Map<String, Boolean> deactivateRoute(
      @PathVariable("channelId") Long channelId, @PathVariable("routeId") Long routeId) {
   ...
   */
  @RequestMapping(
      value = "/channels/{channelId}/routes/{routeId}/activate",
      method = RequestMethod.PUT,
      produces = org.springframework.http.MediaType.APPLICATION_JSON_UTF8_VALUE)
   ...
   */
  @RequestMapping(
      value = "/channels/{channelId}/duplicate-route",
      method = RequestMethod.GET,
      produces = org.springframework.http.MediaType.APPLICATION_JSON_UTF8_VALUE)
  public Map<String, Boolean> isDuplicateRoute(
      @PathVariable("channelId") Long channelId, @RequestParam("routeName") String routeName) {
    Map<String, Boolean> responseMap = new HashMap<String, Boolean>();
    logger.trace(
        "Checking for duplicate route namewith channelId: "
            + channelId
            + " and routeName: "
            + routeName);
    Boolean result = bisRouteService.isRouteNameExists(channelId, routeName);
