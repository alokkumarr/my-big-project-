...
public class CSVReportDataExporter implements IFileExporter {
  ...
  public StringBuffer appendHeader(String[] rowHeader) {
    logger.debug(
        "User activity started here:" + this.getClass().getName() + " appendCSVHeader method");
    ...
    logger
        .debug("User activity ended here:" + this.getClass().getName() + " appendCSVHeader method");
    return csvHeader;
  }
  ...
  public File generateFile(ExportBean exportBean, List<Object> recordRowList) {
    ...
    try {
      ...
      logger.error("Exception occured while writing CSV file:" + this.getClass().getName()
          + " generateCSV method", e);
    } finally {
      try {
        if (writer != null) {
          writer.close();
        ...
        logger.error("Exception occured while closing CSV file:" + this.getClass().getName()
            + " generateCSV method");
      }
    }
