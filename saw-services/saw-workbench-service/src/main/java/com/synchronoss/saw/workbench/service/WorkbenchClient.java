...
public class WorkbenchClient {
  private static final String WORKBENCH_JAR = "/opt/bda/saw-workbench-service/saw-workbench-spark.jar";
  private final Logger log = LoggerFactory.getLogger(getClass().getName());

  public void submit(String livyUri, String root, String project, String component, String config)
      throws Exception {
    LivyClient client = new LivyClientBuilder().setURI(new URI(livyUri)).build();
    log.debug("Uploading Workbench JAR");
    client.uploadJar(new File(WORKBENCH_JAR));
    log.info("Submitting Workbench job");
    JobHandle<Integer> job = client.submit(new WorkbenchJob(root, project, component, config));
