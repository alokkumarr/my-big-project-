...
public class PlayReader {
    ...
    public Swagger read(Class<?> cls) {
        readRoutes(cls, false);
        readModel(cls);
        return getSwagger();
    }

    private void readRoutes(Class<?> cls, boolean readHidden) {
    ...
    private void readModel(Class<?> cls) {

        ApiModel apiModel = cls.getAnnotation(ApiModel.class);
        if (apiModel == null) return;

        Model m = new ModelImpl();
        m.setDescription( apiModel.description() );
        m.setTitle( apiModel.value() );

        Field f[] = cls.getFields();
        Map<String, io.swagger.models.properties.Property> amp_map = null;

        for (int i = 0; i < f.length; i++) {
            ApiModelProperty ap[] =  f[i].getAnnotationsByType(ApiModelProperty.class);
            //check only zero-element
            if (ap != null && ap.length > 0) {
                if (amp_map == null) amp_map = new HashMap<>();

                io.swagger.models.properties.Property mp;
                if (ap[0].dataType().equalsIgnoreCase("string")) {
                    mp = new io.swagger.models.properties.StringProperty();
                } else if (ap[0].dataType().equalsIgnoreCase("integer")) {
                    mp = new io.swagger.models.properties.IntegerProperty();
                } else if (ap[0].dataType().equalsIgnoreCase("long")) {
                    mp = new io.swagger.models.properties.LongProperty();
                } else if (ap[0].dataType().equalsIgnoreCase("array")) {
                    mp = new io.swagger.models.properties.ArrayProperty();
                } else if (ap[0].dataType().equalsIgnoreCase("double")) {
                    mp = new io.swagger.models.properties.DoubleProperty();
                } else if (ap[0].dataType().equalsIgnoreCase("boolean")) {
                    mp = new io.swagger.models.properties.BooleanProperty();
                } else {
                    Logger.warn("Unsupported data type: " + ap[0].dataType());
                    continue;
                }
                mp.setDescription(ap[0].notes());
                mp.setName(ap[0].name());
                mp.setAccess(ap[0].access());
                mp.setRequired(ap[0].required());
                mp.setTitle(ap[0].value());
                amp_map.put(f[i].getName(), mp);
            }
        }
        if ( amp_map != null ) m.setProperties(amp_map);
        swagger.addDefinition(cls.getCanonicalName(), m);
    }
