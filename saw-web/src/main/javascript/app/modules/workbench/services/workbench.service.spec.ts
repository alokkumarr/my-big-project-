...
describe('Workbench Service', () => {
  ...
  it('should return error if dataset request failed', inject(
    [WorkbenchService],
  ...
  it('should get the datasets', inject(
    [WorkbenchService],
    (WorkbenchService: WorkbenchService) => {
      const mockResponse = {
        data: [
          {
            ...
            description: 'Desc 1',
            size: '10000',
            last_updated_time: 'updated_time 1',
            updated_by: 'user 1',
            status: 'status 1',
            data_set_type: 'type 1',
            source_type: 'src 1',
            file_count: 'count 1',
          ...
          },
          {
            ...
            description: 'Desc 2',
            size: '10000',
            last_updated_time: 'updated_time 2',
            updated_by: 'user 2',
            status: 'status 2',
            data_set_type: 'type 2',
            source_type: 'src 2',
            file_count: 'count 2',
  ...
  it('should return error if staging data request failed', inject(
    [WorkbenchService],
    (WorkbenchService: WorkbenchService) => {
      WorkbenchService.getStagingData(
        `${this.wbAPI}/${this.userProject}/raw/directory`
      ).subscribe((res: any) => {
  ...
  it('should get the staging data', inject(
    [WorkbenchService],
    (WorkbenchService: WorkbenchService) => {
      const mockStagingResponse = {
        delimiter: ',',
        description:
          'It is delimited file inspecting to verify & understand the content of file',
        fields: [
          { name: 'DOJ', type: 'date', format: ['YYYY-MM-DD'] },
      ...
      WorkbenchService.getStagingData(
        `${this.wbAPI}/${userProject}/raw/directory`
      ).subscribe((res: any) => {
  ...
  it('should return error if raw preview data request failed', inject(
    [WorkbenchService],
    (WorkbenchService: WorkbenchService) => {
      WorkbenchService.getRawPreviewData(
        `${this.wbAPI}/${userProject}/raw/directory/preview`
      ).subscribe((res: any) => {
  ...
  it('should get the rawPreview data', inject(
    [WorkbenchService],
    (WorkbenchService: WorkbenchService) => {
  ...
  it('should get dataSet details', inject(
    [WorkbenchService],
    (WorkbenchService: WorkbenchService) => {
      let id = 1;
      const dateSetResponse = {
        data: [
          {
            id: 0,
            name: 'DS 1',
  ...
  it('should return error if parsed preview data request failed', inject(
    [WorkbenchService],
    (WorkbenchService: WorkbenchService) => {
      WorkbenchService.getParsedPreviewData(
        `${this.wbAPI}/${userProject}/raw/directory/inspect`
      ).subscribe((res: any) => {
        expect(res.failure.error.type).to.be(
          'ERROR_LOADING_PARSED_PREVIEW_DATA'
        );
      });
    }
  ));

  it('should get the parsed preview data', inject(
    [WorkbenchService],
    (WorkbenchService: WorkbenchService) => {
  ...
  it('createSemantic API should exist and return an Observable', inject(
    [WorkbenchService],
    (workbenchService: WorkbenchService) => {
      expect(
