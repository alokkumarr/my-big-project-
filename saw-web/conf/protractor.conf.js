 ...
 * Defined to be dependent on environment where tests are executed.
 * Running against distribution package in CI requires extended timeouts
 */
...
const extendedImplicitlyWait = webpackHelper.distRun() ? 1200000 : 30000; // = 30 sec; Sometimes element will not appear so fast
...
exports.config = {
  ...
  suites: webpackHelper.distRun() ? {
     ...
     * Note: In the long term there should just be a single set of suites used everywhere (for both
     * continuous integration and local front-end development). However, for now use a separate suite
     * that allows enabling known working tests (working reliably without flakiness) incrementally one
     * by one in continuous integration, while working on fixing the rest.
     */
    authentication: [
