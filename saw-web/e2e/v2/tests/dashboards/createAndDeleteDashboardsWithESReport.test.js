...
describe('Running create and delete dashboards with ES Report in dashboards/createAndDeleteDashboardsWithESReport.test.js', () => {
    ...
    beforeAll(() => {
        logger.info('Starting dashboards/createAndDeleteDashboardsWithEsReport.test.js.....');
    ...
    using(testDataReader.testData['CREATEDELETEDASHBOARDSWITHESREPORT']['dashboards'] 
    ? testDataReader.testData['CREATEDELETEDASHBOARDSWITHESREPORT']['dashboards']
    : {}, (data, id) => {
        it(`${id}:${data.description}`, () => {
            ...
            try{
                ...
                const name = 'AT ' + Constants.ES_REPORT + ' ' + globalVariables.e2eId + '-' + currentTime;
                const description = 'AT Description:' + Constants.ES_REPORT + ' for e2e ' + globalVariables.e2eId + '-' + currentTime;
                const dashboardName = 'AT Dashboard Name' + currentTime;
                const dashboardDescription = 'AT Dashboard description ' + currentTime;
                
                let analysis = new ObserveHelper().addAnalysisByApi(host, token, name, description, Constants.ES_REPORT, null);
            ...
            testId: id,
            data: data,
            feature: 'CREATEDELETEDASHBOARDSWITHESREPORT',
