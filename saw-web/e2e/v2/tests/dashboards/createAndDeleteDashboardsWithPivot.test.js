...
describe('Running create and delete dashboards with Pivot in dashboards/createAndDeleteDashboardsWithPivot.test.js', () => {
    ...
    beforeAll(() => {
        logger.info('Starting dashboards/createAndDeleteDashboardsWithPivot.test.js.....');
    ...
    using(testDataReader.testData['CREATEDELETEDASHBOARDSWITHPIVOT']['dashboards'] 
    ? testDataReader.testData['CREATEDELETEDASHBOARDSWITHPIVOT']['dashboards']
    : {}, (data, id) => {
        it(`${id}:${data.description}`, () => {
            ...
            try{
                ...
                const name = 'AT ' + Constants.PIVOT + ' ' + globalVariables.e2eId + '-' + currentTime;
                const description = 'AT Description:' + Constants.PIVOT + ' for e2e ' + globalVariables.e2eId + '-' + currentTime;
                const dashboardName = 'AT Dashboard Name' + currentTime;
                const dashboardDescription = 'AT Dashboard description ' + currentTime;
                
                let analysis = new ObserveHelper().addAnalysisByApi(host, token, name, description, Constants.PIVOT, null);
            ...
            testId: id,
            data: data,
            feature: 'CREATEDELETEDASHBOARDSWITHPIVOT',
