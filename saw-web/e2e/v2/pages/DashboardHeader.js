...
class DashboardHeader extends GlobalFilters {
  constructor() {
    super();
    this._addDashboardButton = element(
      by.css(`[e2e='dashboard-new-dashboard-button']`)
    );
    this._manualRefreshButton = element(
      by.css(`[e2e='dashboard-manual-refresh-button']`)
    );
    this._deleteDashboardButton = element(
      by.css(`[e2e='dashboard-delete-dashboard-button']`)
    );
    this._downloadDashboardButton = element(
      by.css(`[e2e='dashboard-download-dashboard-button']`)
    );
    this._editDashboardButton = element(
      by.css(`[e2e='dashboard-edit-dashboard-button']`)
    );
    this._dashboardAction = action =>
      element(by.xpath(`//span[contains(text(),"${action}")]`));
    this._openGlobalFilterButton = element(
      by.css(`[e2e='dashboard-open-global-filters-button']`)
    );
  }
