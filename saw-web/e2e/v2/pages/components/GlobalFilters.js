...
class GlobalFilters {
  constructor() {
    ...
    this._cancelFilter = element(
      by.css('[e2e="dashboard-global-filters-cancel-button"]')
    );
    this._numberSlider = element(
    ...
    try {
      dashboardGlobalFilters.forEach(currentFilter => {
        if (currentFilter.name.toLowerCase() === 'string') {
          browser.sleep(2000);
          expect(this._stringFilter.getAttribute('value')).toBe(
            currentFilter.value
          );
      ...
      dashboardGlobalFilters.forEach(currentFilter => {
        if (currentFilter.name.toLowerCase() === 'string') {
          ...
          commonFunctions.clickOnElement(
            this._stringFilterValue(currentFilter.value)
          );
        } else if (currentFilter.name.toLowerCase() === 'date') {
          browser.sleep(2000);
