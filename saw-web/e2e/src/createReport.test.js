...
describe('Create report type analysis: createAndDeleteReport.test.js', () => {
  ...
  beforeAll(function() {
    ...
    browser
      .manage()
      .timeouts()
      .implicitlyWait(protractorConf.timeouts.extendedImplicitlyWait);
  });
  ...
  using(
    testDataReader.testData['CREATEREPORT']['createReportDataProvider'],
    function(data, description) {
      it(
        'Should apply filter to Report ' +
          description +
          ' testDataMetaInfo: ' +
          JSON.stringify({
            test: description,
            feature: 'CREATEREPORT',
            dp: 'createReportDataProvider'
          }),
        () => {
          ...
          commonFunctions.waitFor.elementToBeClickable(
            reportDesigner.filterBtn
          );
          reportDesigner.filterBtn.click();

          commonFunctions.waitFor.elementToBeClickable(
            designModePage.filterWindow.addFilter(tables[0].name)
          );
          ...
          commonFunctions.waitFor.elementToBeClickable(
            designModePage.filterWindow.number.operatorDropDownItem(
              filterOperator
            )
          );
          designModePage.filterWindow.number
            .operatorDropDownItem(filterOperator)
            .click();
          filterInput.clear();
          ...
          let filterDisplayed =
            fieldName + ': ' + filterOperator + ' ' + filterValue; //This is new change to app
          ...
          commonFunctions.waitFor
            .elementToBePresent(createdAnalysis)
