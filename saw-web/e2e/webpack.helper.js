...
function generateFailedTests(dir) {
  ...
  if (fs.existsSync('target/e2e/testData/processed/processedFiles.json')) {
    ...
    oldProcessedFiles.forEach(function(oldProcessedFile) {
      let filesInOld = JSON.parse(fs.readFileSync('target/e2e/testData/processed/'+oldProcessedFile,'utf8'));
    ...
    fs.writeFileSync('target/e2e/testData/processed/'+oldProcessedFile, JSON.stringify(oldProcessedXmlFiles), { encoding: 'utf8' });

    // delete old file
    fs.unlinkSync('target/e2e/testData/processed/processedFiles.json');
    // Create new file which is actual failed tests
    fs.writeFileSync('target/e2e/testData/processed/processedFiles.json', JSON.stringify(filesToProcess), { encoding: 'utf8' });

  } else {
    filesToProcess = files;
    // write processedFiles
    fs.writeFileSync('target/e2e/testData/processed/processedFiles.json', JSON.stringify(files), { encoding: 'utf8' });
  }
  ...
  if (fs.existsSync('target/e2e/testData/failed/failedTests.json')) {
    ...
    oldFailedTests.forEach(function(oldFailedTest) {
      ...
      oldFailedJsonData.push(JSON.parse(fs.readFileSync('target/e2e/testData/failed/'+oldFailedTest,'utf8')));

    });

    let oldFailedFile = 'oldFailedTests_'+new Date().getTime()+'.json';
    fs.writeFileSync('target/e2e/testData/failed/'+oldFailedFile, JSON.stringify(oldFailedJsonData), { encoding: 'utf8' });
...
module.exports = {
    ...
    if (fs.existsSync('target/e2e/url.json')) {
      ...
      process.argv.forEach(function (val) {
        if(val.includes('--baseUrl')) {
          ...
          fs.writeFileSync('target/e2e/url.json', JSON.stringify(urlObject), { encoding: 'utf8' });
          return;
        }
      });
    }
  ...
  getTestData: () => {
    if (fs.existsSync('target/e2e/testData/failed/failedTests.json')) {
      ...
      let data = JSON.parse(fs.readFileSync('target/e2e/testData/failed/failedTests.json','utf8'));
      ...
      if (fs.existsSync('target/e2e/suite.json')) {
        ...
        process.argv.forEach(function (val) {
          if(val.includes('--suite')) {
            ...
            fs.writeFileSync('target/e2e/suite.json', JSON.stringify(suiteObject), { encoding: 'utf8' });
            return;
          }
        });
      }
