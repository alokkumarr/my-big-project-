...
for(inps in rcomp_inputs_df$dataSet) {
  ...
  input_dataset_name <- paste(project, input_dataset_name, sep = "::")
  ...
  X <- reader(sc, name = input_df_name, path = input_dataset_folder, type = input_data_format, 
...
tmp_subscr_mms_tbl <- AGGR_MMS_TBL_DF

tmp_config_latest_subscr_info <- CONFIG_ACS_TBL_DF

SUBSCR_MMS_TBL_DF <- full_join(tmp_config_latest_subscr_info, tmp_subscr_mms_tbl, by = c("EVENT_TIMESTAMP", "SUBSCRIBER_ID")) %>%
  select(., EVENT_TIMESTAMP, SUBSCRIBER_ID, CLIENT_VERSION, TERMINAL_MODEL, TERMINAL_VENDOR, STATUS,
         MESSAGE_COUNT, MESSAGE_SENT_COUNT, MESSAGE_RECEIVED_COUNT, MESSAGE_TRANSFER_DURATION, MESSAGE_SIZE,
         FAILED_MESSAGE_COUNT) %>%
  group_by(EVENT_TIMESTAMP, SUBSCRIBER_ID) %>%
  summarise(CLIENT_VERSION = max(CLIENT_VERSION)
            ,TERMINAL_MODEL = max(TERMINAL_MODEL)
            ,TERMINAL_VENDOR = max(TERMINAL_VENDOR)
            ,STATUS = max(STATUS)
            ,MESSAGE_COUNT = sum(MESSAGE_COUNT)
            ,MESSAGE_SENT_COUNT = sum(MESSAGE_SENT_COUNT)
            ,MESSAGE_RECEIVED_COUNT = sum(MESSAGE_RECEIVED_COUNT)
            ,MESSAGE_TRANSFER_DURATION = avg(MESSAGE_TRANSFER_DURATION)
            ,MESSAGE_SIZE = mean(MESSAGE_SIZE)
            ,FAILED_MESSAGE_COUNT = sum(FAILED_MESSAGE_COUNT)
            )

SUBSCR_CONFIG_ACS_TBL_DF <- tmp_config_latest_subscr_info %>%
  select(., EVENT_TIMESTAMP, SUBSCRIBER_ID, CLIENT_VERSION, TERMINAL_MODEL, TERMINAL_VENDOR, STATUS)
...
# Get Project Output folder path
...
for (row in 1:nrow(rcomp_outputs_df)) {
  output_dataset_name <- as.character(rcomp_outputs_df[row, "dataSet"])
  output_catalog <- as.character(rcomp_outputs_df[row, "catalog"])
  output_mode <- as.character(rcomp_outputs_df[row, "mode"])
  output_format <- as.character(rcomp_outputs_df[row, "format"])
  output_repart_numb <- as.numeric(rcomp_outputs_df[row, "numberOfFiles"])
 ...
 writer(df = eval(parse(text=output_df_name)), path = output_dataset_folder, 
                     mode = output_mode, type = output_format, partitions = output_repart_numb) 
 
 #sip_add_dataset_details()
 sip_add_dataset(output_path = output_dataset_folder, output_format = output_format, script = "r_subscr_mms_script.R",
