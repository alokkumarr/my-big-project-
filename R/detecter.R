...
detect.data.frame <- function(df,
  ...
  checkmate::assert_number(frequency, lower = 2)
  checkmate::assert_choice(direction, c("pos", "both", "neg"))
  checkmate::assert_number(alpha, lower = 0, upper = 1.0)
  checkmate::assert_number(max_anoms, lower = 0, upper = 1.0)
  checkmate::assert_number(trend_window, lower = 0, upper = 1.0)
