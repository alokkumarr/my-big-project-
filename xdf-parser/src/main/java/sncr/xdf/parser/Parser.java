...
public class Parser extends Component implements WithMovableResult, WithSparkContext, WithDataSetService {
    ...
    int parse(String mode){
        ...
        resultDataDesc.add(new MoveDataDescriptor(tempDir, outputDataSetLocation, outputDataSetName, mode, outputFormat));
        ...
        for (FileStatus file : files) {
            if (file.isFile()) {
                ...
                if (parseSingleFile(file.getPath(), new Path(tempPath)) == 0) {
                    resultDataDesc.add(new MoveDataDescriptor(tempPath, outputDataSetLocation, outputDataSetName, mode,
                            outputFormat));
                }
            }
        }
