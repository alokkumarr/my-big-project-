...
==== Create and Retrieve of RTIS App Keys

RTIS requires app key to submit the events. Follow the below steps to create app key:
...
.. Bath Size - It measures batch size in total bytes instead of the number of messages.
...
NOTE: App key is auto generated, user does not allow to specify app key.
