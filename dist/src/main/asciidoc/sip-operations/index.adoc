...
=== SIP Security : External APIs

SIP security provides external API to manage `users` and their `roles`,`categories`,`subcategories`,  `privileges` and `data security key`

==== API for creating  and fetching the users.

These  APIs managed by the admin only.While creating the user  middleName, securityGroupName are non-manadatory
and for ID3 login passowrd is non-manadatory.

Create User API [POST]: `https://{hostname}/sip/security/auth/admin/cust/manage/external/users/create`
...
----
{
    ...
    "middleName": null,
    "lastName": "Deshagani",
    "masterLoginId": "Anil10.Deshagani",
    "password": "Sawsyncnewuser1!",
    "email": "sawadminq@synchronoss.com",
    "activeStatusInd": true,
...
Fetch API [GET]: `https://{hostname}/sip/security/auth/admin/cust/manage/external/users/fetch`
sample response:
----
{
    "error": null,
    ...
    "users": [
        {
            "activeStatusInd": true,
            "customerCode": "SYNCHRONOSS",
            "email": "sysadmin@synchronoss.com",
            "firstName": "system",
            "middleName": "sncr",
            "lastName": "admin",
...
----
==== API for Role, Categories, Subcategories, Privileges

Add API [POST]: `https://{hostname}/sip/security/auth/external/createRoleCategoryPrivilege`

This API managed by the admin only. This API contains the Roles, Categories, subCategories and Privileges which allow options for below
json hierarchy to create the respective options roles, categories, subCategories and privileges if `"autoCreate": true`.

Below are the few consideration while create/fetch roles, categories, subcategories ans privileges.

- Product name and Module name can't be created using API, it should be already exists in SIP. In case it doesn't exists then error will
be thrown `Product and Module does not exist for this user.` as precondition check failed.
- By default `autoCreate` will be false (ie. not be created). If in case respective privileges and options already existing then API will be
return the information as details already exists in SIP.
- Below are the validation to create role, categories, subcategories and privileges.
.. Customer code should be matched with the access token.
.. Role/Category/Subcategory name should not allowed special symbol except [_ and -].
.. Role type should be ADMIN or USER only
.. Given privilege should be valid.
...
Fetch API [GET]: `https://{hostname}/sip/security/auth/external/fetchRoleCategoryPrivilege`

This api will fetch the details (roles, categories/subcategories and privileges) based upon the `product name, module name` and `role name` name.
