...
== Batch Ingestion : Retry parameters

The below parameters in Batch Ingestion Service gives the control the configurations
related to resiliency. To change this parameters in the environment please go to below path

/opt/bda/saw-batch-ingestion/conf/application.yml

max.attempts : 3 (maximum number of retries then it will declare it as failure)
retry.delay: 5000 (Interval between the retries in milliseconds)
retry.diff.mins: 1 (Interval between the retries to recover from inconsistent state of transfer in minutes)
