...
sip_executors_reserved_port properties can be set in `saw-config` file for starting port
...
The below is one such example to call onboard-customer with its required parameters.
    ...
    shell:>onboard-customer --C SOFTBANK0001 --P SOFTBANK --PC workbench --E abc@xyz.com --F ABC --M XY --L Z
    1
...
Ex : onboard-customer --C SOFTBANK0001 --P SOFTBANK --PC workbench --E abc@xyz.com --F ABC --M 'X Y' --L Z
