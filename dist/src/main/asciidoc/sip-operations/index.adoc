...
There are some use cases where semantic id which are created long
back are missed during semantic migration. This breaks the relationship
of semantic and reports, as the reports would store the info semantic id
...
1. Login to saw node as root user
2. Navigate to /opt/bda/sip-metadata-service
3. Run the following command by providing the following parameters.
bash semanticIdMigration <base maprdb path> <semantic json metadata table name>
<analysis json metadata table name>
E.g.: bash semanticIdMigration maprfs:///var/sip semanticDataStore analysisMetadata
