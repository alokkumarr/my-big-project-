...
related to resiliency. To change this parameters in the environment which are as follows

To control what should be the time difference
to declare a data in the system is corrupted, & needs to be downloaded again (default is 5 mins)

sip_bis_retry_diffs_in_minutes=5

To control where to verify the partial file check on the source or not. Default is true.
sip_bis_recheck_file_modified=true
