...
. Install and configure Apache Spark version 2.3.2 from the MapR
...
package in the `sip-config` file, which defaults to installing SAW on
...
. Get the SAW release package (named `sip-*.tgz`)
...
to the below SAW environment configuration (`sip-config`).

    saw_spark_master_url=yarn
    saw_spark_yarn_queue_regular=saw-regular
    saw_spark_yarn_queue_fast=saw-fast
    saw_spark_yarn_jars=/opt/mapr/spark/spark-2.3.2/jars
...
Where `<sip-service>` is one of the SAW systemd services (for example
...
The below is one such example to call onboard-customer with its required parameters.
    ...
    shell:>onboard-customer --C SNCR0001 --P SAWDEMO --PC workbench --E abc@xyz.com --F ABC --M XY --L Z
    1
...
Note : A parameter containing a space in between can be enclosed in either single quotes('') or double quotes("").
        ...
        Ex : onboard-customer --C SNCR0001 --P SAWDEMO --PC workbench --E abc@xyz.com --F ABC --M 'X Y' --L Z
...
After generating the key, add it to the SAW environment configuration ({{sip-config}}) in the {{saw_security_sso_secret}} parameter and redeploy.
