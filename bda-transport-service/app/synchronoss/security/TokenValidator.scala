...
class TokenValidator(token: String) extends HTTPRequest{
  ...
  {
    val bypass =  TSConfig.security_settings.getBoolean("bypass")
    if (bypass) return true

    val sec_host = TSConfig.security_settings.getString("host")
    val sec_port = TSConfig.security_settings.getInt("port")
    val sec_path = TSConfig.security_settings.getString("path")
    val sec_timeout = TSConfig.security_settings.getInt("timeout")
    var result = false
