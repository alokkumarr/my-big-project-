...
import synchronoss.metadata.MetadataDictionary
...
class ESQueryHandler (ext: Extractor) extends HTTPRequest {
  ...
  {
    val es_ip = TSConfig.es_conf.getString("host")
    val es_port = TSConfig.es_conf.getInt("port")
