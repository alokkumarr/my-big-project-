...
module.exports = {
  ...
  token: baseUrl => {
    const payload = {
      masterLoginId: users.masterAdmin.loginId,
      password: users.masterAdmin.password
    };
    const q = urlParser.parse(baseUrl, true);
    url = Constants.HTTP_PROTOCOL + '://' + q.host + '/'; // API base url
    return 'Bearer '.concat(
      JSON.parse(
        request('POST', url + 'security/doAuthenticate', {
          json: payload
        }).getBody()
      ).aToken
    );
  ...
  usersRolesPrivilegesCategories: token => {
    ...
    roles.admin.roleId = getRoleIdByRoleName(
      rolesListWithAdmin,
      roles.admin.roleName
    );
    roles.userOne.roleId = getRoleIdByRoleName(
      rolesListWithUser,
    ...
    generateSubCategory(
      categories.observe,
      subCategories.observeSubCategory,
      observeCategoriesList,
      token,
      2
    ); //moduleID =2 for observe.
    generatePrivilege(
      privileges.all,
      roles.admin,
      categories.observe,
...
function generateRole(role, token) {
  ...
  return apiCall.post(
    url + 'security/auth/admin/cust/manage/roles/add',
    payload,
    token
  ).roles; // return list of all
  // roles
}

function getRoleIdByRoleName(rolesList, roleName) {
  return getValueFromListByKeyValue(
    rolesList,
    'roleName',
    roleName,
    'roleSysId'
...
function generateUser(user, roleId, token) {
  ...
  return apiCall.post(
    url + 'security/auth/admin/cust/manage/users/add',
    payload,
    token
...
function generateCategory(category, token, module = null) {
  ...
  const categoriesList = apiCall.post(
    url + 'security/auth/admin/cust/manage/categories/add',
    payload,
    token
  ).categories;
...
function generateSubCategory(
  ...
  parentCategory.id = getValueFromListByKeyValue(
    categoriesList,
    'categoryName',
    parentCategory.name,
    'categoryId'
  );
  parentCategory.type = getValueFromListByKeyValue(
    categoriesList,
    'categoryName',
    parentCategory.name,
    'categoryType'
  );
  parentCategory.code = getValueFromListByKeyValue(
    categoriesList,
    'categoryName',
    parentCategory.name,
    'categoryCode'
  ...
  // get list of all categories
  const categoriesListWithSubCategory = apiCall.post(
    url + 'security/auth/admin/cust/manage/categories/add',
    payload,
    token
  ).categories;
  // get list of all sub-categories
  let subCategoriesList = getSubCategoryListByCategoryName(
    categoriesListWithSubCategory,
    parentCategory.name
  );

  subCategory.id = getSubCategoryIdBySubCategoryName(
    subCategoriesList,
    subCategory.name
  );
...
function generatePrivilege(
  ...
  return apiCall.post(
    url + 'security/auth/admin/cust/manage/privileges/upsert',
    payload,
    token
  );
}
...
function getValueFromListByKeyValue(list, inputKey, inputValue, getValueOfKey) {
  ...
  if (returnValue == null) {
    throw new Error(
      'There is no ' + inputKey + ' in list with value ' + inputValue
    );
  }
  return returnValue;
}
...
function generateChart(semanticId, dataSetName, user, subCategory, token) {
  ...
  // Get chart ID
  const chartID = apiCall.post(url + 'services/analysis', createPayload, token)
    .contents.analyze[0].id;
        ...
        analyze: [
          {
            ...
            sqlBuilder: {
              ...
              sorts: [
                { columnName: 'string.keyword', type: 'string', order: 'asc' },
                {
                  columnName: 'date',
                  type: 'date',
                  order: 'asc'
...
function getSemanticId(dataSetName, token) {
  ...
  let response = apiCall.get(
    url + 'services/internal/semantic/md?projectId=workbench',
    token
  );
