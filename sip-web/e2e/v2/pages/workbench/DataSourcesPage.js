...
class DataSourcesPage extends DeleteModel {
  constructor() {
    ...
    this._createdChannelHostName = name => element(by.xpath(`//*[@e2e="${name}"]/following::td[1]`));

    this._channelActivateDeactivate = (name, status) =>
      element(by.xpath(`//*[@e2e="${name}"]/following::button[@e2e="${status}"][1]`));

    this._channelNameDetail = element(by.css(`[e2e="channel-name-detail-panel"]`));
    this._showPwd = element(by.css(`[e2e="show-password-ds"]`));
    this._hidePwd = element(by.css(`[e2e="hide-password-ds"]`));

    this._channelAttributes = name => element(by.xpath(`//*[@e2e="${name}"]/following::span[1]`));
    ...
    this._routeStatus = status =>
      element(by.xpath(`//button[@e2e="route-active-inactive-btn"]/descendant::span[contains(text(),"${status}")]`));
    this._back = element(by.xpath(`//div[contains(@class,"cdk-overlay-backdrop")]`));
    this._routeSearchInput = element(by.xpath(`(//input[@aria-label="Search in data grid"])[2]`));
    this._routeLogs = element(by.css(`[e2e="view-route-logs-btn"]`));
    this._getRouteScheduleRowValueOf = ColumnName =>
      element(by.xpath(`(//*[@e2e="route-log-${ColumnName}"])[position()=last()]`));

    this._closeRouteLogsModel = element(by.xpath(`//button[contains(@class,"close-button")]`));
    this._routeName = value => element(by.css(`[e2e='route-routeName-${value}']`));
    this._filePattern = value => element(by.css(`[e2e='route-filePattern-${value}']`));
    ...
    this._jobRouteName = value => element(by.css(`[e2e='job-routeName-${value}']`));
    this._jobFilePattern = value => element(by.css(`[e2e='job-filePattern-${value}']`));
    this._jobIdByRouteName = value =>
      element(by.xpath(`(//*[@e2e='job-routeName-${value}'])[1]/preceding::div[contains(@e2e,'job-jobId')]/a`));

    //Job details
    this._jobLogFilePattern = value => element(by.css(`[e2e='job-log-filePattern-${value}']`));
    this._jobLogFileName = value => element(by.css(`[e2e='job-log-fileName-${value}']`));
    this._jobLogProcessState = value => element(by.css(`[e2e='job-log-bisProcessState-${value}']`));
    this._jobLogFileStatus = value => element(by.css(`[e2e='job-log-mflFileStatus-${value}']`));
  ...
  verifyChannelDetailsInListView(channelName, hostName, status) {
    commonFunctions.waitFor.elementToBeVisible(this._createdChannel(channelName));
    expect(this._createdChannelHostName(channelName).getText()).toEqual(hostName);
    commonFunctions.waitFor.elementToBeVisible(this._channelActivateDeactivate(channelName, status));
  }
  ...
  verifyCurrentDisplayedChannel(channelInfo) {
    ...
    commonFunctions.validateText(this._channelNameDetail, channelInfo.channelName);
    commonFunctions.validateText(this._channelAttributes('host-name'), channelInfo.sftpHost);
    commonFunctions.validateText(this._channelAttributes('access-type'), channelInfo.access);
    commonFunctions.validateText(this._channelAttributes('port-number'), channelInfo.sftpPort);
    commonFunctions.validateText(this._channelAttributes('created-by'), channelInfo.created);
    commonFunctions.validateText(this._channelAttributes('user-name'), channelInfo.sftpUser);
    commonFunctions.clickOnElement(this._showPwd);
    commonFunctions.waitFor.elementToBeVisible(this._hidePwd);
    commonFunctions.validateText(this._channelAttributes('password-name'), channelInfo.sftpPwd);
  ...
  deActivateChannel(name) {
    ...
    commonFunctions.waitFor.elementToBeVisible(this._channelActivateDeactivate(name, 0));
  }

  activateChannel(name) {
    commonFunctions.clickOnElement(this._channelActivateDeactivate(name, 0));
    commonFunctions.waitFor.elementToBeVisible(this._channelActivateDeactivate(name, 1));
  }
  ...
  scheduleVerification(routeInfo) {
    expect(this._jobLogFilePattern(routeInfo.filePattern).isDisplayed()).toBeTruthy();
    expect(this._jobLogFileName(`${routeInfo.source}/${routeInfo.fileName}`).isDisplayed()).toBeTruthy();
    expect(this._jobLogFileStatus(`SUCCESS`).isDisplayed()).toBeTruthy();
    expect(this._jobLogProcessState(`DATA_RECEIVED`).isDisplayed()).toBeTruthy();
  }
  ...
  verifyCurrentDisplayedApiChannel(channelInfo) {
    commonFunctions.waitFor.elementToBeVisible(this._channelNameDetail);
    commonFunctions.validateText(this._channelNameDetail, channelInfo.channelName);
    commonFunctions.validateText(this._channelAttributes('host-name'), channelInfo.hostName);
    commonFunctions.validateText(this._channelAttributes('port-number'), channelInfo.port);
    commonFunctions.validateText(this._channelAttributes('created-by'), channelInfo.created);
