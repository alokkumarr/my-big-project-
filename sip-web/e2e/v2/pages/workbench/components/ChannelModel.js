...
class ChannelModel extends TestConnectivity {
  constructor() {
    ...
    this._headerName = index =>
      element(by.css(`[e2e='e2e-name-of-header-${index}']`));
    this._requestBody = element(by.css(`[e2e='e2e-body-parameter-content']`));
    this._headerNameOptions = (index, name) =>
      element(by.css(`[e2e="header-name-options-${index}-${name}"]`));
    this._headerValue = index =>
      element(by.css(`[e2e="e2e-value-of-header-${index}"]`));
    this._headerValueOptions = (index, value) =>
      element(by.css(`[e2e="header-value-options-${index}-${value}"]`));
    this._queryTab = element(by.css(`[e2e="e2e-query-tab"]`));
    this._addParameterBtn = element(by.css(`[e2e="e2e-add-query-param-btn"]`));
    this._queryParamName = index =>
      element(by.css(`[e2e="e2e-name-of-query-param-${index}"]`));
    this._queryParamValue = index =>
      element(by.css(`[e2e="e2e-value-of-query-parameter-${index}"]`));
    this._allHeaders = element.all(
      by.xpath(`//button[contains(@e2e,'e2e-remove-header')]`)
    );
    this._removeHeader = element(by.css(`[e2e="e2e-remove-header-0"]`));
    this._allQueryParams = element.all(
      by.xpath(`//button[contains(@e2e,'e2e-remove-query-param-btn')]`)
    );
    this._removeQueryParam = element(
      by.css(`[e2e="e2e-remove-query-param-btn-0"]`)
    );
  }
  ...
  addHeaders(headers) {
    ...
    for (const [key, value] of Object.entries(headers)) {
      ...
      // to handle the Authorization header with token.
      // Token is very long and normal sendkyes takes lot of time to enter this much text
      if (key === 'Authorization' && value.startsWith('Bearer')) {
        browser.executeScript(
          `arguments[0].value='${value}';`,
          this._headerValue(index)
        );
        this._headerValue(index).sendKeys(protractor.Key.BACK_SPACE);
        this._headerValue(index).sendKeys(value.charAt(value.length - 1));
      } else {
