...
In the following chunk, the `month` function in the lubrdate package R is used to extract the month from the date. The ouput is consistent to the Hive `month` UDF and code is consistent for dataset objects.
...
In the next chunk the `week` function in lubridate is used to extract the week number from a date, however, this function does not have a direct Hive corallary. Instead the `weekofyear` Hive UDF produces the same output as the lubridate `week` function.
...
Below is a few examples of a2munge functions with the R and Spark methods shown. The first example is the `summariser` where the two methods are very similiar. 
